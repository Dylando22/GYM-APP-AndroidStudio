...
public class WorkoutsFragment extends Fragment {
    public WorkoutsFragment(){super(R.layout.create_workout_fragment);}

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        WorkoutsViewModel viewModel = new ViewModelProvider(getActivity()).get(WorkoutsViewModel.class);
        ObservableArrayList workoutEntries = viewModel.getEntries();
        WorkoutsAdapter adapter = new WorkoutsAdapter(
                workoutEntries,
                entry -> {
                    viewModel.setCurrentWorkout(entry);
                    getActivity().getSupportFragmentManager().beginTransaction()
                            .replace(R.id.HomeFragment, SingleWorkoutItemFragment.class, null)
        ...
        RecyclerView recyclerView = view.findViewById(R.id.workouts_recycler);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        recyclerView.setAdapter(adapter);
        view.findViewById(R.id.AddNewWorkout).setOnClickListener(fab -> {
            viewModel.setCurrentWorkout(null);
            getActivity().getSupportFragmentManager().beginTransaction()
